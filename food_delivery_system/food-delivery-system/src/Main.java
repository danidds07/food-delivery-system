...
public class Main {
        ...
        if (option == JOptionPane.OK_OPTION) {
            ...
            if(!name.isEmpty() && !email.isEmpty()) {
                ...
                if (c != null) {
                    ...
                    JOptionPane.showMessageDialog(frame, "Error saving client to database.", "Error", JOptionPane.ERROR_MESSAGE);
                }
            } else {
    ...
    private static void registerRestaurant(JFrame frame) {
        ...
        if (option == JOptionPane.OK_OPTION) {
            ...
            if (r == null) {
                JOptionPane.showMessageDialog(frame, "Error saving restaurant to database.", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
                ...
                try {
                    ...
                    for (int i = 0; i < qtd; i++) {
                        ...
                        if (itemOpt == JOptionPane.OK_OPTION) {
                            Menu item = sistema.addMenuItem(r, descField.getText().trim(), Integer.parseInt(priceField.getText().trim()));
                            if (item == null) {
                                JOptionPane.showMessageDialog(frame, "Error saving menu item to database.", "Error", JOptionPane.ERROR_MESSAGE);
                            }
                        }
                    }
        ...
        if (option == JOptionPane.OK_OPTION) {
            ...
            if(!name.isEmpty() && !email.isEmpty() && !vehicle.isEmpty()) {
                ...
                if (d != null) {
                    ...
                    JOptionPane.showMessageDialog(frame, "Error saving deliveryman to database.", "Error", JOptionPane.ERROR_MESSAGE);
                }
            } else {
    ...
    private static void newOrder(JFrame frame) {
        List<Client> clients = sistema.getClients();
        if (clients.isEmpty()) {
        ...
        List<Restaurant> restaurants = sistema.getRestaurants();
        ...
        while (true) {
            ...
            for (int i = 0; i < menu.size(); i++) {
                ...
                itemArray[i + 1] = (i + 1) + " - " + item.getDesc() + " | R$ " + item.getPrice();
            }
        ...
        if (!chosenItems.isEmpty()) {
            Order newOrder = sistema.makeOrder(selectedClient, selectedRestaurant, chosenItems);
            if (newOrder != null) {
    ...
    private static void assignDeliveryman(JFrame frame) {
        String orders = sistema.getOrderListString();
        if (orders.isEmpty()) {
        ...
        if (idStr != null && !idStr.trim().isEmpty()) {
            try {
                ...
                boolean success = sistema.assignDeliveryman(selectedOrder);
                JOptionPane.showMessageDialog(frame, success ? "Deliveryman assigned!" : "No available deliverymen or error assigning.");
            } catch (NumberFormatException e) {
        ...
        if (idStr != null && !idStr.trim().isEmpty()) {
            try {
                ...
                if (selectedStatus != null) {
                    ...
                    if (newStatus != null) {
                        boolean updated = sistema.updateStatusOrder(orderToUpdate, newStatus);
                        if (updated) {
                            JOptionPane.showMessageDialog(frame, "Status updated!");
                        } else {
                            JOptionPane.showMessageDialog(frame, "Error updating status in database.", "Error", JOptionPane.ERROR_MESSAGE);
                        }
                    }
                }
            } catch (NumberFormatException e) {
    ...
    private static void showOrderList(JFrame frame) {
        String output = sistema.getOrderListString();
